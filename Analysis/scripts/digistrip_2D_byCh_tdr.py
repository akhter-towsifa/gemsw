...
for ch in range(2, 8):
  ...
  xAxis.SetTitle("Digi Strips")

  yAxis = h_list[ch-2].GetYaxis()
  yAxis.SetTitleOffset(0)
  yAxis.SetTitleSize(0.04)
  yAxis.SetTitle("i#eta")

  event.Project("h{ch}".format(ch=ch), "rechit_location[4]:digi_strip", "rechit_location[2]=={ch}".format(ch=ch))
  ...
  canvas.SaveAs("{r}/digistrip_ch{ch}_2D.png".format(ch=ch, r=run))
